...
```
```{r density_scatterplot, echo=FALSE}
sp_lm(aes(x=quality, y=density), 'Quality x Density - Scatterplot')
```

```{r quality_no_outliers, echo=FALSE}
ggplot(subset(df, density < quantile(df$density, 0.99)), aes(x=quality, y=density)) +
  geom_point(alpha=1/10, 
             position=position_jitter(h=0), 
             size=4) +
  geom_smooth(method='lm') +
...
df$sweetness <- ifelse(df$residual.sugar <= 4.0, 'DRY',
               ifelse(df$residual.sugar > 4.0 & df$residual.sugar <= 12.0, 'MEDIUM DRY',
               ifelse(df$residual.sugar > 12.0 & df$residual.sugar <= 45.0, 'MEDIUM',
               ifelse(df$residual.sugar > 45.0, 'SWEET', 'NULL'))))
```

```{r sweetness_barplot, echo=FALSE}
bar_plot(aes(x=df$sweetness, fill=df$sweetness), 'Sweetness Frequency Barplot')
```

```{r sweetness_scatterplot, echo=FALSE}
...
ggplot(subset(df, total.sulfur.dioxide <= 50), aes(x=quality, y=total.sulfur.dioxide)) +
...
ggplot(subset(df, total.sulfur.dioxide > 50 & total.sulfur.dioxide < quantile(df$total.sulfur.dioxide, 0.95)), aes(x=quality, y=total.sulfur.dioxide)) +
...
```{r ph_scatterplot, echo=FALSE}
ggplot(df, aes(x=quality, y=residual.sugar, color=factor(acidity))) +
  geom_point(position=position_jitter(h=0), alpha=1/2, size=1.5) +
  scale_colour_brewer(palette='Set2') +
  geom_smooth(method='lm') +
  xlab('Quality') +
...
sample_sp(aes(x=sample_df$density, y=sample_df$alcohol, color=factor(sample_df$quality)), 
                  '[SAMPLE] Density x Alcohol by Quality (color)')
```

```{r d_a_q_fw, echo=FALSE, warning=FALSE}
box_plot(aes(x=df$density, y=df$alcohol), 'Density x Alcohol by Quality (facet wrap)', ~df$quality)
```
...
ggplot(subset(sample_df, sample_df$free.sulfur.dioxide < quantile(sample_df$free.sulfur.dioxide, 0.99)), aes(x=free.sulfur.dioxide, y=total.sulfur.dioxide,
                                                                                         color=factor(quality))) +
    geom_point(position=position_jitter(h=0), size=2) +
    scale_colour_brewer(palette='Set2') +
    ggtitle('[SAMPLE] Free Sulfur Dioxide x Total Sulfur Dioxide by Quality (color) 2')
```
...
sample_sp(aes(x=density, y=residual.sugar, color=factor(sweetness)), 
                  '[SAMPLE] Density x Residual Sugar by Sweetness (color)')
```
...
ggplot(df, aes(x=df$quality, y=df$alcohol)) +
  ...
  ylab('Alcohol (%)') +
  ggtitle('Quality x Alcohol - Scatterplot') +
  theme_minimal()
```
...
plot2_1 <- ggplot(df, aes(x=sweetness, fill=sweetness)) +
  geom_bar() +
  scale_fill_brewer(palette='Set2') +
...
plot2_2 <- ggplot(subset(df, residual.sugar < quantile(df$residual.sugar, 0.95)), aes(x=quality, y=residual.sugar, color=factor(sweetness))) +
  geom_point(position=position_jitter(h=0), alpha=1/2, size=1.5) +
  scale_colour_brewer(palette='Set2') +
  geom_smooth(method='lm') +
  labs(x='Quality (0 to 10 grade)', y='Residual Sugar (g/L)', color='Sweetness')
  ggtitle('Quality x Residual Sugar by Sweetness (color)') +
  theme_minimal()
...
ggplot(subset(sample_df, density < quantile(sample_df$density, 0.99)), aes(x=density, y=alcohol, color=factor(quality))) +
  geom_point(position=position_jitter(h=0), size=2) +
  scale_colour_brewer(palette='Set2') +
  labs(x='Density (g/L)', y='Alcohol (%)', color='Quality') +
  ggtitle('[SAMPLE] Density x Alcohol by Quality (color)') +
  theme_minimal()
```
